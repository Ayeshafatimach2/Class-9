...
let current_users: string[] = ['Ayesha', 'Fatima', 'Rabia', 'Zanib', 'Eman'];
let new_users: string[] = ['Saman', 'Eman', 'Sidra', 'Fatima', 'Muntaha'];

for (let index = 0; index < new_users.length; index++) {

    if (current_users.includes(new_users[index])) {

        console.log(new_users[index], 'this user name is already exist, please try again later');
